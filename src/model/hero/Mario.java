/**
 * the purpose of this class is to create a Mario object that manages his state, behavior, movement, and attributes
 * while the game status is active (or saves his state while paused)
 */
...
public class Mario extends GameObject{
    ...
    public void jump(GameEngine engine) {
        if(!isJumping() && !isCrouching() && !isFalling()){
            setJumping(true);
            setVelY(13);
            engine.playJump();
        }
    ...
    public void crouch(GameEngine engine) {
        if(!isJumping() && !isCrouching() && !isFalling()) {
            setCrouching(true);
